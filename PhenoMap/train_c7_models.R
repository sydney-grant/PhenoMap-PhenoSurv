...
pathways <- read.csv("GSVA_Results_PANCAN_C7.csv")
#pathways <- pathways[grepl("REACTOME", pathways[,1]),]
#pathways <- pathways[,-1]

if (start == 4801 | start == 1600){
stop <- nrow(pathways)
}
...
genes_pathways_hallmark <- read.csv(paste0("c7_genefilter_", split_num, "IMPACT.csv"))[,-1]
print(head(genes_pathways_hallmark))
...
calc_pathway <- function(pathway, input_mutations, input_cna, train_indis, gene_df, model){
  pathname <- as.character(pathway[,length(pathway)])
...
pathways <- pathways[start:stop,]
pathways$pathname <- as.factor(pathway_names[start:stop])
genes_pathways_hallmark <- subset(genes_pathways_hallmark, Pathway %in% pathways$pathname)
...
print("Saving data...")
write.csv(train_preds, paste0("train_predictions_FINAL_", split_num, "_", start, "_", stop, "_c7_IMPACT.csv"))
write.csv(test_preds, paste0("test_predictions_FINAL_", split_num,  "_", start, "_", stop, "_c7_IMPACT.csv"))
#saveRDS(model_list, paste0("c7_trained_models_", start, "_", stop, "IMPACT.rds"))
