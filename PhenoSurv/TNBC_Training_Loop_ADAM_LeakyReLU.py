...
path_train = pd.read_csv("~//Documents//Survival VAE//breast_top_predicted_0.25_IMPACT_tnbc.csv")
survival = pd.read_csv("~//Documents//Survival VAE//breast_clin_top_0.25_IMPACT_tnbc.csv")
filtered_survival = survival.dropna(subset=['DFS_MONTHS', 'DFS_STATUS'])

path_train = path_train[path_train['PatientID'].isin(filtered_survival['PATIENT_ID'])]
filtered_survival = filtered_survival[filtered_survival['PATIENT_ID'].isin(path_train['PatientID'])]


filtered_survival = filtered_survival[['PATIENT_ID', 'DFS_STATUS', 'DFS_MONTHS']]
filtered_survival = filtered_survival.rename(columns={'PATIENT_ID':'PatientID','DFS_STATUS':'Status', 'DFS_MONTHS':'Survival_in_days'})

path_train = pd.merge(path_train, filtered_survival, on='PatientID')

path_x_train = path_train.drop(columns = ['Unnamed: 0','PatientID', 'Status', 'Survival_in_days'])
...
### Grouping pathways into phenotypes
...
#### Making 5-fold training and test splits
...
df.to_csv('~//Documents//Survival VAE//FINAL PHENOSURV//pathways_trainingloop_Adam_LeakyReLU_breast_tnbc_FINAL.csv', index=False)
