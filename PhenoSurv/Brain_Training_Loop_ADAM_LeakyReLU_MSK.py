...
path_val = pd.read_csv("~//Documents//Survival VAE//Validation Data//Brain_MSK_Scores_IMPACT.csv")
path_val = path_val.dropna(subset=['PFS_MONTHS', 'PFS_STATUS'])
path_x_val = path_val.drop(columns = ['Unnamed: 0','PatientID', 'PFS_MONTHS', 'PFS_STATUS', 'OS_MONTHS', 'OS_STATUS'])
path_y_val = path_val[['PFS_STATUS', 'PFS_MONTHS']]
path_y_val = path_y_val.rename(columns={'PFS_STATUS':'Status', 'PFS_MONTHS':'Survival_in_days'})
...
df.to_csv('~//Documents//Survival VAE//FINAL PHENOSURV//pathways_trainingloop_Adam_LeakyReLU_brain_MSK_FINAL.csv', index=False)
